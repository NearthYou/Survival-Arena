...
| 구간 | 시간 | 반경 | 바깥 피해 |
| --- | ---: | ---: | ---: |
| 1단계 | 0초에서 120초 | 128에서 96 | 초당 2 |
| 2단계 | 120초에서 240초 | 96에서 64 | 초당 4 |
| 3단계 | 240초에서 360초 | 64에서 32 | 초당 8 |
| 4단계 | 360초에서 480초 | 32에서 8 | 초당 16 |
| Sudden death | 480초에서 600초 | 8에서 0 | 초당 32 |
...
- zone 반경은 128, 96, 64, 32, 8, 0
- 시간, 이동 속도, perception, 체력, 무기 피해와 cooldown은 유지

종료 tick acceptance는 바꾸지 않는다. 확대 후 같은 seed를 다시 실행해 14,400에서 18,000 tick을 실제로 통과하는지 검증한다.
