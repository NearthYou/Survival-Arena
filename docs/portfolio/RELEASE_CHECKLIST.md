...
| LFS object 실제 가용성 | `partial` | 현재 checkout의 세 runtime asset은 pointer가 아닌 파일이지만 공개 후보 checkout에서 committed pointer, object-store SHA 및 size, worktree hydration을 함께 대조한 dated proof를 아직 남기지 않았다. 라이선스 및 자산 목록 완료와 object availability를 같은 gate로 취급하지 않는다. |
...
5. 공개 후보 checkout에서 세 runtime asset의 committed LFS pointer, object-store 파일과 hydrated worktree SHA 및 size를 read-only로 대조하고 dated proof를 남긴다.
